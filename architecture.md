...
生效   = 设置值存在即为准:非空用之,空 = 停用该模块的
         触发入口(路由返回 None);仅 key 缺失才回落
         模块声明值。route 每次输入现读 SettingsHost——
         Immediate,无 try-apply 回调、无宿主副作用
事务   = apply_setting_inner 在 Immediate 分支内对
         trigger_keys 做单例处理:trim 归一 + Core 校验,
         跳过 module.try_apply_settings(§42 的 validate →
         commit → persist 链不变)
校验   = 空(含 trim 后为空)= 停用,直接放行;非空要求
         不含空白、≤16 字符、不与其他模块的生效触发词
         冲突;失败不 commit,UI 留在编辑态并回显错误
...
容错   = 持久化值之间的历史冲突(如新版本引入的模块默认
         ...
         不 panic、不自动改写用户设置
明确不做 = 默认模块触发词(§82 无前缀语义不动,App 也
           因此不可停用,§65)、大小写归一(保持精确匹配,
           用户设什么就是什么)、模块整体的启用/禁用开关
           (空触发词已覆盖键盘入口,进程内加载照常)
```
