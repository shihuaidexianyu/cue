...
- **文件搜索**:`/` 进入文件模式,经本机 [Everything](https://www.voidtools.com/) 1.4 秒搜全盘文件与文件夹(原生语法 `ext:`、路径过滤等原样透传);默认排除系统目录与"工具内脏"(Windows、Program Files、ProgramData、任意用户的 AppData、node_modules、.git、包缓存等)——结果里只有你的工作文件;名单是 `%LOCALAPPDATA%\CUE\modules\file\data\excluded-paths.toml`(TOML 数组,每行一个片段),设置页选中「排除名单文件」回车即用默认编辑器打开,保存后下一次查询生效;输入含 `\` 的显式路径时不过滤;Everything 未运行时行内提示、不影响其余功能
...
- **可设置**(托盘右键 → 设置):唤起热键、失焦隐藏、开机自启、游戏模式、**各模块触发词**(`b`/`/`/`>` 可改成你喜欢的词,留空即停用该入口)——事务式生效,改失败不留半成品
