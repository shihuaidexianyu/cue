...
- **文件搜索**:`/` 进入文件模式,经本机 [Everything](https://www.voidtools.com/) 1.4 秒搜全盘文件与文件夹(原生语法 `ext:`、路径过滤等原样透传);默认排除系统与依赖目录(Windows、Program Files、node_modules、.git 等,可在设置中关闭;输入含 `\` 的显式路径时不过滤)——结果里只有你的工作文件;Everything 未运行时行内提示、不影响其余功能
