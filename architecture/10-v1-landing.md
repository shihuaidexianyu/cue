...
> **本节决定已随 §149 裁撤**(2026-09-19):强制英文整体移除,键盘
> 状态交还用户。正文保留作历史;裁撤动机与行为影响见 §149。
...
增补（2026-08-25，空态呈现时机）：Core 语义不变——输入变化立即
清空 results/selection，stale 结果永不可激活（§102）。但"清空"
不再立刻入画：提交在途（session 的 `results_pending`）且视图手里
有行时，**保持绘制上一批行**，提交到达——空结果也算提交——才
换画；"No results"只来自已提交的空结果集，不再在每次击键的间隙
闪现。此前每次击键都把结果区闪成空态（`/` 文件搜索提交间隔
50–140 ms，逐键频闪），选中带上边缘时隐时现，被看成"分割线
抖动、粗细变化"（E2E 逐键截图实证：修复前带在相邻帧间
ROWS/空交替，修复后单调）。安全性不损失：保持窗口期 Core 的
selection 是 None，Enter 什么都激活不了；呈现至多滞后一次提交，
与所有 launcher 的常规行为相同。

增补（2026-08-25，客户区尺寸纪律）：**`place_on_active_monitor`
必须把客户区（client area）定为设计逻辑尺寸，而不是窗口矩形**。
`SetWindowPos` 的尺寸含 DWM 不可见缩放边框（150% 缩放下宽
22px、高 13px），直接喂 640×450 的换算值会让客户区缩水成
938×662 物理 px（441.33 逻辑高）。此时满 5 行结果的内容（646.5
物理 px）超出内容盒（644 px）2.5px，flex 收缩按 min-content 取舍
——结果列表不可压（min-content=555），输入行/间隙/分割线可压
——输入行被压矮 2px，分割线上移 1.33 逻辑 px 且被 taffy 取整
从 2px 挤成 1px。这正是"候选不足 5 个时分割线变粗下移"的真身：
<5 行才是正常布局，满 5 行是被压缩的（布局探针逐帧记录：
5 行态 `y=58.667 h=0.667`，其余 `y=60 h=1.333`）。修复：第一步
SetWindowPos（移动+DPI 同步）后量取 `GetWindowRect` 与
`GetClientRect` 的实际差（边框随 DPI 缩放，必须在落到目标显示器
之后量），第二步把客户区定为 640×450 逻辑、位置按客户区居中。
修后所有状态分割线恒为 `y=60 h=1.333`（物理 90、2px），E2E 逐键
截图 34 帧像素位置完全一致。同理，任何按内容预算窗口尺寸的
布局（设置页 8 行×36px 等）都以客户区为准。
