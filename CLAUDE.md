...
- **Product:** `Alt+Space` opens the launcher; plain input searches apps (AppModule); `b` + space searches Chromium bookmarks (§117); `/` searches files via Everything (§31–33, §118 — coverage = Everything's index scope; Everything absent → inline error, graceful degradation); `>` runs fixed system actions (§126 — lock/sleep/hibernate/logoff/restart/shutdown/empty-recycle-bin); `Tab` on a selected result opens the action menu — App: 打开/以管理员身份运行/打开所在位置 (last two Win32-only), File: 打开/打开所在文件夹/复制路径, Bookmark: 打开/复制链接 (§18, §119). The search box forces English input (§107); Chinese apps are found via pinyin/initials, so AppModule's pinyin index is the only CJK path. CUE is a **single-instance** app (§113): a second process signals the first to show/focus, then exits.
