...
- Package for distribution: `scripts/package.ps1` (release build → `dist/CUE-<ver>-win-x64.zip` with per-user `install.ps1`/`uninstall.ps1`; `-Sign` runs `scripts/sign.ps1` first — self-signed dev cert only proves integrity, public distribution needs an OV cert). If Inno Setup 6 (`ISCC.exe`) is installed it also compiles `scripts/setup.iss` → `dist/CUE-Setup-<ver>.exe`: per-user wizard installer (免管理员), zh-CN (vendored `scripts/lang/ChineseSimplified.isl`, from the matching Inno tag) + en via `ShowLanguageDialog=auto`, detects a running CUE via the §113 mutex (`AppMutex=Local\CUE.SingleInstance`), registers an HKCU uninstall entry, optional 开机自启 task writes the same HKCU Run key as the in-app toggle. `core.start_on_boot` (§36) is the in-app autostart toggle — host callback writes the HKCU Run key, same §53/§112 sync-exception pattern as `apply_hotkey`.
