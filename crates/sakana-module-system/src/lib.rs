//! sakana-module-system —— SystemModule(§126)。
//!
...
impl Module for SystemModule {
    ...
    fn load(&mut self, ctx: ModuleContext) -> Result<(), ModuleError> {
        ...
        self.icons = ACTIONS
            .iter()
            .filter_map(|s| {
                sakana_util_win::glyph::render_glyph(s.glyph, GLYPH_RGB).map(|i| (s.glyph, i))
            })
            .collect();
...
mod exec {
    ...
    fn shutdown(reboot: bool) -> Result<(), ModuleError> {
        let msg = if reboot {
            w!("sakana:30 秒后重启。取消请运行 shutdown /a")
        } else {
            w!("sakana:30 秒后关机。取消请运行 shutdown /a")
        };
        unsafe {
