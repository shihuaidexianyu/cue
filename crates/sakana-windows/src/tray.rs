...
//!
//! 图标优先用 exe 内嵌的品牌资源(crate::icon,assets/sakana.ico);
...
use crate::host::{HostMsg, WM_SAKANA_TRAY, WM_SAKANA_TRAY_CMD};
...
pub fn add(host: HWND) -> Result<(), Error> {
    ...
    for (i, c) in "sakana".encode_utf16().enumerate() {
        tip[i] = c;
    }
    let data = NOTIFYICONDATAW {
        ...
        uCallbackMessage: WM_SAKANA_TRAY,
        hIcon: icon,
        szTip: tip,
        ..Default::default()
    };
...
/// host WndProc 在 WM_SAKANA_TRAY 时调用。未设 NOTIFYICON_VERSION_4,
/// lparam 直接是鼠标消息。
...
unsafe fn show_menu(host: HWND) {
    unsafe {
        ...
        let _ = AppendMenuW(menu, MF_STRING, TRAY_CMD_SHOW, w!("显示 sakana"));
        let _ = AppendMenuW(menu, MF_STRING, TRAY_CMD_SETTINGS, w!("设置"));
        let _ = AppendMenuW(menu, MF_STRING, TRAY_CMD_QUIT, w!("退出 sakana"));
        ...
        if cmd.as_bool() {
            // 延迟分发(见 WM_SAKANA_TRAY_CMD 注释):此刻同步调 handler
            // 会让刚唤起的窗口在菜单拆除时被抢前台。
            let _ = PostMessageW(
                Some(host),
                WM_SAKANA_TRAY_CMD,
...
/// WM_SAKANA_TRAY_CMD 的 wParam → HostMsg(host WndProc 侧分发)。
