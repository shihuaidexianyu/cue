...
//! 提取本身由 sakana-util-win::icon 提供。
//!
//! 线程模型:module 自有 worker 线程串行提取(与 AppModule 图标
//! 管线同款);负缓存防重试风暴;缓存超界整体清空(重建很便宜)。

use sakana_protocol::{IconImage, ItemId, ModuleEvent, ModuleEventSink, ResultIcon};
...
fn extract(key: &str) -> Option<IconImage> {
    if let Some(p) = key.strip_prefix("path:") {
        sakana_util_win::icon::extract_file_icon(Path::new(p))
    } else if let Some(ext) = key.strip_prefix("ext:") {
        sakana_util_win::icon::extract_virtual_icon(&format!("x.{ext}"), FILE_ATTRIBUTE_NORMAL)
    } else {
        None
    }
}
...
fn worker_loop(
    ...
    if generic.get().is_none()
        && let Some(icons) = (|| {
            Some(FileIcons {
                folder: Arc::new(sakana_util_win::icon::extract_virtual_icon(
                    "folder",
                    FILE_ATTRIBUTE_DIRECTORY,
                )?),
                file: Arc::new(sakana_util_win::icon::extract_virtual_icon(
                    "file",
