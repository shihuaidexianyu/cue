...
fn default_exclude_file() -> PathBuf {
    std::env::var("LOCALAPPDATA")
        .map(|p| PathBuf::from(p).join("sakana"))
        .unwrap_or_else(|_| PathBuf::from("sakana"))
        .join("modules")
        .join("file")
        .join("data")
    ...
    fn icon_for(&self, entry: &FileEntry) -> Option<ResultIcon> {
        ...
        match self.icons.get() {
            ...
            None => {
                ...
                sakana_util_win::glyph::cached_glyph(cp, sakana_util_win::glyph::DEFAULT_RGB)
    ...
    fn settings_schema(&self) -> SettingsSchema {
        vec![
            ...
            SettingSpec {
                ...
                description: Some(
                    "分号分隔的目录列表,加入文件索引;默认已覆盖用户目录与桌面/文档/下载;改动重启 sakana 后生效"
                        .into(),
                ),
    ...
    fn activate(&mut self, item: &ModuleItem, action: ActionId) -> ActivationFuture {
        ...
        Box::pin(async move {
            ...
            let result = match action {
                ActionId::PRIMARY => sakana_util_win::shell::shell_execute(&entry.path, None, None),
                ACTION_REVEAL => sakana_util_win::shell::reveal_in_explorer(&entry.path),
                ACTION_COPY_PATH => sakana_util_win::clipboard::set_text(&entry.path),
...
mod tests {
    ...
    fn seed_file_roundtrips_into_fragments() {
        let dir = std::env::temp_dir().join(format!("sakana-file-seed-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join(EXCLUDE_FILE_NAME);
        seed_exclude_file(&file).unwrap();
        let content = std::fs::read_to_string(&file).unwrap();
        assert!(content.starts_with("# sakana"));
    ...
    fn refreshed_fragments_follow_mtime() {
        let dir = std::env::temp_dir().join(format!("sakana-file-mtime-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join("list.toml");
    ...
    fn malformed_toml_keeps_previous_fragments() {
        let dir = std::env::temp_dir().join(format!("sakana-file-bad-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join("list.toml");
        std::fs::write(&file, "excluded = ['\\alpha\\']\n").unwrap();
