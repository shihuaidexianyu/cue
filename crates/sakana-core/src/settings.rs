...
impl SettingsHost {
    pub fn new(
        ...
        let log_path = file
            .as_deref()
            .and_then(|f| f.parent())
            .map(|p| p.join(sakana_protocol::log::LOG_FILE_NAME))
            .unwrap_or_else(|| PathBuf::from(sakana_protocol::log::LOG_FILE_NAME));
        let mut host = Self {
            specs: Vec::new(),
            values: HashMap::new(),
            file,
...
fn core_specs(log_path: PathBuf) -> Vec<SettingSpec> {
    vec![
        SettingSpec {
            ...
            label: "诊断日志".into(),
            description: Some(
                "回车用系统默认程序打开;启动/唤起/模块告警等诊断行,超 1MB 自动滚动一代".into(),
            ),
        ...
        SettingSpec {
            ...
            label: "开机自启".into(),
            description: Some("登录 Windows 时自动启动 sakana(写入当前用户的 Run 注册表项)".into()),
            kind: SettingKind::Bool,
...
mod tests {
    ...
    fn persisted_value_overrides_default() {
        let dir = std::env::temp_dir().join(format!("sakana-settings-test-{}", std::process::id()));
    ...
    fn corrupt_file_falls_back_to_defaults() {
        let dir =
            std::env::temp_dir().join(format!("sakana-settings-test2-{}", std::process::id()));
        let file = dir.join("settings.tsv");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn legacy_game_mode_key_migrates_to_dnd_mode() {
        let dir =
            std::env::temp_dir().join(format!("sakana-settings-test4-{}", std::process::id()));
        let file = dir.join("settings.tsv");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn commit_persists_and_reloads() {
        let dir =
            std::env::temp_dir().join(format!("sakana-settings-test3-{}", std::process::id()));
        let file = dir.join("settings.tsv");
