//! 启动:Win32 = ShellExecuteEx(sakana-util-win);Packaged =
...
pub fn launch(target: &LaunchTarget) -> Result<(), ModuleError> {
    match target {
        LaunchTarget::Win32 {
            ...
            args,
            working_dir,
        } => sakana_util_win::shell::shell_execute(
            &exe.to_string_lossy(),
...
pub fn launch_elevated(target: &LaunchTarget) -> Result<(), ModuleError> {
    match target {
        LaunchTarget::Win32 {
            ...
            args,
            working_dir,
        } => sakana_util_win::shell::shell_execute_elevated(
            &exe.to_string_lossy(),
            (!args.is_empty()).then_some(&**args),
            working_dir.as_deref(),
        ),
...
pub fn reveal_location(target: &LaunchTarget) -> Result<(), ModuleError> {
    match target {
        LaunchTarget::Win32 { exe, .. } => {
            sakana_util_win::shell::reveal_in_explorer(&exe.to_string_lossy())
        }
        LaunchTarget::Packaged { aumid } => Err(ModuleError::ActivationFailed(format!(
...
fn launch_packaged(aumid: &str) -> Result<(), ModuleError> {
    ...
    let aumid_w = sakana_util_win::shell::to_wide(aumid);
    unsafe {
