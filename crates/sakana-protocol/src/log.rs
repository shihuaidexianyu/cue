...
pub const LOG_FILE_NAME: &str = "sakana.log";
pub const LOG_FILE_OLD: &str = "sakana.log.old";
/// 超界滚动:启动时 sakana.log > 1MB 就改名 .old(覆盖上一代)从零开始,
...
mod tests {
    ...
    fn init_rotates_oversize_log_and_roundtrips() {
        let dir = std::env::temp_dir().join(format!("sakana-log-test-{}", std::process::id()));
