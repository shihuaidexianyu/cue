...
/// LauncherModule —— 能参与 Launcher 输入交互的 Module。
///
...
pub struct LauncherDescriptor {
    /// `None` 表示无前缀模态;一个 Launcher 只能有一个 default Module。
    pub trigger: Option<String>,
    pub is_default: bool,
}

///
/// v0.2:`generation` 已删除——staleness 是 Core 的 bookkeeping,
...
/// Module 只回答"结果是什么";有效性判定全部在 Core 侧。
#[derive(Debug)]
pub struct QueryResponse {
    pub items: Vec<ModuleItem>,
}

/// 必须 `Send + 'static`;创建 Future 本身 < 1 ms,创建时不得触碰
...
/// Activation 的错误在 `ModuleOutcome` 内表达,不单独设 `Err`。
