//! 行图标:每行显示来源浏览器的图标(Edge/Chrome exe 提取,
//! 提取走 sakana-util-win::icon),不做网站 favicon(Flow 默认关;要读
...
pub fn load_browser_icons() -> HashMap<Browser, Arc<IconImage>> {
    ...
    for browser in [Browser::Edge, Browser::Chrome] {
        if let Some(icon) = browser
            .exe_path()
            .and_then(|exe| sakana_util_win::icon::extract_file_icon(&exe))
        {
...
mod tests {
    ...
    fn edge_icon_extracts() {
        ...
        let icon = sakana_util_win::icon::extract_file_icon(&exe).expect("edge icon extraction");
        assert_eq!(
            (icon.width, icon.height),
            (
                sakana_util_win::icon::ICON_SIZE,
                sakana_util_win::icon::ICON_SIZE
            )
        );
        assert!(
